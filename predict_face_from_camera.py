...
def main(capturedImage, model_path, classifier_output_path):
    """
...
#     plt.show()
    ...
    with tf.Session(config=tf.ConfigProto(log_device_placement=False)) as sess:
        ...
        aligned_images,boundary_boxes=detect_and_align_faces(capturedImage) # contains list of aligned face
...
#         print(imgs)
        ...
        recognize_face(emb_array,classifier_filename,capturedImage,boundary_boxes)
...
def recognize_face(emb_array,classifier_filename,capturedImage,boundary_boxes):
...
if __name__=='__main__':
    ...
    while True:
        ...
        #print(ret)
        cv2.imshow("image",image)
        key_pressed=cv2.waitKey(1) # 1 millisecond waiting time
        if key_pressed==13 or key_pressed==32: #unicode for enter key and space key
           print("KeyPressed")
           break
    cv2.destroyAllWindows()
    ...
    main(image,
         model_path=driveLocation+'20180402-114759_model/20180402-114759.pb',
